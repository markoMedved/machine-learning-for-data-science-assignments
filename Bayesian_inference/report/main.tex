...
To perform inference, we used the Markov Chain Monte Carlo (MCMC)
 ...
 generated 1000 posterior samples following a burn-in phase of 1000
  iterations, which were discarded.
...
For point predictions, we match the summary statistic to the corresponding loss function:
\begin{itemize}
    \item \textbf{Squared error loss:} use the predictive mean
    \item \textbf{Absolute error loss:} use the predictive median
    \item \textbf{Accuracy:} use the predictive mode
\end{itemize}
